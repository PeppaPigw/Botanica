...
void main() {
  ...
  testWidgets('tapping a room pulse card navigates to garden room query path',
    ...
    await tester.scrollUntilVisible(roomPulseFinder, 120);
    await tester.tap(roomPulseFinder);
    await tester.pumpAndSettle();

    expect(find.text('/garden?room=Bathroom'), findsOneWidget);
  });

  testWidgets(
      'tapping an unassigned room pulse card navigates to unassigned garden query path',
      (WidgetTester tester) async {
    final plants = <Plant>[
      _plant('p1', 'Nameless', ''),
      _plant('p2', 'Fern', 'Bathroom'),
    ...
    final tasks = <TaskInstance>[
      _task(id: 't1', plantId: 'p1', dueAt: DateTime(2026, 3, 6, 7)),
      _task(id: 't2', plantId: 'p1', dueAt: DateTime(2026, 3, 5, 7)),
    ];
    final logs = <CareLog>[
      _log('l1', 'p1', DateTime(2026, 2, 10, 8)),
      _log('l2', 'p2', DateTime(2026, 3, 6, 8)),
    ];
    ...
    expect(find.text('/garden?room=Unassigned'), findsOneWidget);
  });
}
